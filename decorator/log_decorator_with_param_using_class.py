class log:
    def __init__(self, to='console', file_path='log'):
        if to == 'console':
            self.log_func = print
        else:
            log_file = open(file_path, 'rt')
            self.log_func = log_file.write

    def __call__(self, func):
        def _wrapper(*args, **kwargs):
            self.log_func(f'input của hàm {func} là: {args} {kwargs}')
            try:
                result = func(*args, **kwargs)
                self.log_func(f'output của hàm {func} là {result}')
